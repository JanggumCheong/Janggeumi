...
```jsonc
"purchase": {
  "headline": "좋은 수박 고르는 법",  // h1(검색어). 화면 비노출. 프론트가 "좋은 {재료} 고르는 법"으로 생성 가능
...
**필터 축 규칙 (보관 구조의 핵심):**
- 필터 축(`filters[].key`)은 **재료별 가변**이다(수박=상태·장소·기간 / 감자=장소·빛·함께둠 / 대파=손질·장소·형태). 재료명·필터를 화면에 하드코딩하지 않는다.
- 축은 **백엔드 `storage_filters`의 `section_id`가 원천**이다(예: `fs_cut_status`→`form`, `fs_storage_location`→`place`). 프론트 축은 여기에 맞춘다.
- **최대 3축을 지향한다.** 보관 결정은 단순해야 한다 — 축이 많으면 오히려 못 고른다. 4축 이상이면 정말 필요한지 되묻고 합친다.
- **기간(period)은 재료 공통 필드가 아니라 "관심사일 때만 등장하는 하나의 축"이다.** 수박은 기간(단기/장기)이 결정축이지만, **감자·대파는 기간이 관심사가 아니다**(감자=빛·함께둠, 대파=손질·형태가 결정축). 그래서 method에 `duration` 같은 **고정 기간 필드를 두지 않는다** — 기간이 관심사인 재료에서만 `filters`에 period 축을 두고 `tags`로 태깅한다.
- **기간은 직관적 라벨(단기/장기)로 표현한다.** "7~10일" 같은 숫자 문자열이 아니라 사용자가 바로 이해하는 버킷으로. 다른 축들과 동일하게 필터+뱃지로 동작한다.

```jsonc
"storage": {
  "headline": "수박 보관 방법 비교",
  "intro": "내 상황에 맞는 방법을 찾아보세요.",
  "filters": [                     // ★ 재료별 가변 축 — 최대 3축 지향. 수박은 기간이 관심사라 period 포함
    { "key": "form",   "label": "상태",      "options": ["통째로", "자른 뒤"] },
    { "key": "place",  "label": "보관 장소",  "options": ["실온", "냉장", "냉동"] },
    { "key": "period", "label": "보관 기간",  "options": ["단기", "장기"] }  // 직관 라벨. 감자·대파엔 이 축 없음
  ],
  "methods": [
    {
      "id": "room-whole",
      ...
      "recommended": true,
      "tags": { "form": "통째로", "place": "실온", "period": "단기" },  // 축과 1:1 태깅(=뱃지=필터). duration 고정필드 없음
      ...
      "comment": {                 // 코멘트 (선택). 화자를 데이터로 — 지금은 장금이(curator)
        "author": { "name": "장금이", "type": "curator" },
        "text": "사 온 날은 무조건 이 방법! 냉장 넣기 전 하루는 실온이 당도에 좋아요."
      },
...
- `comment` — **검증된 큐레이션 한마디**(선택). `{ author: { name, type }, text }` 객체. 사용자 후기(로그인·UGC)를 대체하는 신뢰 근거. 화자를 **데이터로** 들고 있어(`author.type`: curator=장금이 / ugc=유저 / official=전문가) 화면이 화자명을 하드코딩하지 않고 확장된다. 지금은 전부 curator(장금이). 처리탭 `recipe.author`와 동일한 `AuthorSchema`를 공유. 방법 카드 하단에 "👩‍🍳 {name} —"로 조용히 노출(방법 비교가 주인공, 코멘트는 보조 한 줄). 각 방법마다 그 방법 고유의 실용 팁을 담는다. → design-system.md 8-3 K1.
