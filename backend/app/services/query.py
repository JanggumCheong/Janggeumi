...
def _ingredient_slug(ingredient_id: Optional[str]) -> Optional[str]:
...
def _format_home_recommendation(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": row.get("id"),
        "slug": _ingredient_slug(row.get("id")),
        "href": _ingredient_href(row.get("id")),
        "name": row.get("name"),
...
def get_recent_views_for_user(user_id: str) -> Any:
    ...
    for row in rows or []:
        ...
        slug = _ingredient_slug(ingredient_id)
        href = _ingredient_href(ingredient_id)
    ...
    try:
        ...
        for row in recent_rows.data:
            ...
            if ing:
                recent_views.append({
                    "id": ing.get("id"),
                    "slug": _ingredient_slug(ing.get("id")),
                    "href": _ingredient_href(ing.get("id")),
                    "name": ing.get("name"),
